...
Specifically, it needs to report the number of characters from the beginning of the buffer to the end of 
...
for line in lines:
    if line != '':        
        for char in line[:(n-1)]:
            chars.append(char)
        for char in line[(n-1):]:    
            chars.append(char)
            dist_chars = set(chars)
            dist_count = len(dist_chars)
            if dist_count != n:
                chars.pop(0)
                counter += 1
            else:
...
Your device's communication system is correctly detecting packets, but still isn't working. It looks like 
