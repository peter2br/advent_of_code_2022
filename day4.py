...
For the first few pairs, this list means:
    ...
    Within the first pair of Elves, the first Elf was assigned sections 2-4 (sections 2, 3, and 4), while 
    the second Elf was assigned sections 6-8 (sections 6, 7, 8).
...
fully contains 3-7, and 6-6 is fully contained by 4-6. In pairs where one assignment fully contains the 
other, one Elf in the pair would be exclusively cleaning sections their partner will already be cleaning, 
...
for line in lines:
    if line != '':
        ...
        if int(min2) in range(int(min1), int(max1)+1) and int(max2) in range(int(min1), int(max1)+1):
            count += 1
        elif int(min1) in range(int(min2), int(max2)+1) and int(max1) in range(int(min2), int(max2)+1):
            count += 1
        else:
            pass
...
for line in lines:
    if line != '':
        ...
        if int(min2) in range(int(min1), int(max1)+1) or int(max2) in range(int(min1), int(max1)+1):
            count += 1
        elif int(min1) in range(int(min2), int(max2)+1) or int(max1) in range(int(min2), int(max2)+1):
            count += 1
        else:
            pass
