...
stored in stacks of marked crates, but because the needed supplies are buried under many other crates, the 
...
her which crate will end up where, and they want to be ready to unload them as soon as possible so they can 
embark.
...
Then, both crates are moved from stack 2 to stack 1. Again, because crates are moved one at a time, crate 
C ends up below crate M:
...
are C in stack 1, M in stack 2, and Z in stack 3, so you should combine these together and give the Elves 
the message CMZ.
...
board_nums = board_data.pop()
#print(board_nums)

for line in range(len(board_nums)):
    if board_nums[line] != ' ':
        board[board_nums[line]] = []
        location[board_nums[line]] = line
        ordering.append(board_nums[line]) 

for line in reversed(board_data):
    for key in location.keys():
        if line[location[key]] != ' ':
            board[key].append(line[location[key]])

board2 = copy.deepcopy(board)

for line in instructions:
    instruction_vals = re.findall(r'(\d+)', line)
    count_move = int(instruction_vals[0])
    from_id = instruction_vals[1]
    to_id = instruction_vals[2]

    for line in range(count_move):
        pop_vals = board[from_id].pop()
        add_vals = board[to_id].append(pop_vals)

    board2[to_id] += board2[from_id][-count_move:]
    board2[from_id] = board2[from_id][:-count_move]

for line in ordering:
    print(board[line][-1], end = '')

### PART 2 ###
print()
for line in ordering:
    print(board2[line][-1], end = '')
