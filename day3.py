...
three Elves. That is, if a group's badge is item type B, then all three Elves will have item type B somewhere 
...
for line in lines:
    if line != '':
        if counter == 1:
            ...
            list3 = line
            for char in list1:
                if char in list2 and char in list3:
                    #print(char)
                    if ord(char) > 64 and ord(char) < 91:
                        val = ord(char) - 38
                    if ord(char) > 96 and ord(char) < 123:
